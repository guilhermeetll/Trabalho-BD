...
async def list_agencias_distinct():
...
@router.get("/total")
async def get_total_financiamentos():
...
async def get_financiamento(codigo_processo: str):
...
async def update_financiamento(codigo_processo: str, fin: FinanciamentoCreate):
...
async def delete_financiamento(codigo_processo: str):
